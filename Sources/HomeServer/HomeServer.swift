            ...
            Group {
                ...
                case .list:
                    if sessionService.sessions.isEmpty {
                        ConnectionView(savedSession: nil, onConnect: { session in
                ...
                case .editor(let session):
                    ConnectionView(savedSession: session, onConnect: { newSession in
...
struct GlassTextField: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            Label(title, systemImage: icon).font(.caption).foregroundStyle(.secondary).padding(.leading, 4)
            TextField("", text: $text).textFieldStyle(.plain).padding(10).background(Color.primary.opacity(0.05)).cornerRadius(8)
        }
    }
}
...
class TunnelManager: ObservableObject {
    ...
    func startTunnel(session: SavedSession, ports: [RemotePort], password: String?) async {
        ...
        var args = ["-N", "-o", "StrictHostKeyChecking=accept-new", "-o", "ServerAliveInterval=15", "-o", "ExitOnForwardFailure=yes"]
            ...
            HStack {
                ...
                Button(action: onNew) {
                    Image(systemName: "plus").font(.headline).padding(10)
                        ...
                        .overlay(Circle().stroke(Color.accentColor.opacity(isHoveringPlus ? 0.6 : 0.3), lineWidth: 1))
...
struct ConnectionView: View {
    ...
    let onCancel: (() -> Void)?
    init(savedSession: SavedSession?, onConnect: @escaping (SavedSession) -> Void, onCancel: (() -> Void)?) {
        self.onConnect = onConnect; self.onCancel = onCancel
        if let session = savedSession {
            _id = State(initialValue: session.id); _name = State(initialValue: session.name)
            _ipAddress = State(initialValue: session.ipAddress); _username = State(initialValue: session.username)
            _selectedKeyPath = State(initialValue: session.keyPath); _environment = State(initialValue: session.environment)
        } else { _id = State(initialValue: UUID()) }
    }
    var body: some View {
        VStack(spacing: 25) {
            HStack {
                Button(action: { onCancel?() }) { Image(systemName: "chevron.left").padding(8).background(Color.primary.opacity(0.05)).clipShape(Circle()) }.buttonStyle(.plain).opacity(onCancel == nil ? 0 : 1)
                Spacer(); Text(name.isEmpty ? "New Server" : name).font(.headline); Spacer(); Color.clear.frame(width: 30, height: 30)
            }
            VStack(alignment: .leading, spacing: 20) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    Menu {
                        Text("Select a key...").tag(""); ForEach(availableKeys, id: \.self) { key in Button(key) { selectedKeyPath = key } }
                        Button("Manual Entry...") { selectedKeyPath = "MANUAL" }
                    } label: { HStack { Text(selectedKeyPath.isEmpty ? "Select Key" : (URL(string: selectedKeyPath)?.lastPathComponent ?? selectedKeyPath)); Spacer(); Image(systemName: "chevron.up.chevron.down").font(.caption) }.padding(10).background(Color.primary.opacity(0.05)).cornerRadius(8) }.menuStyle(.borderlessButton)
                }
                if selectedKeyPath == "MANUAL" { GlassTextField(icon: "folder", title: "Path", text: $selectedKeyPath) }
            }.padding(25).background(.ultraThinMaterial).cornerRadius(16)
            HStack(spacing: 15) {
                Button("Save") { _ = save(); onCancel?() }.padding().background(Color.primary.opacity(0.05)).cornerRadius(12)
                Button(action: connect) {
                    HStack { if isConnecting { ProgressView().controlSize(.small) } else { Text("Connect"); Image(systemName: "arrow.right") } }
                        .font(.headline).frame(maxWidth: .infinity).padding().background(Color.accentColor).foregroundStyle(.white).cornerRadius(12)
    ...
    private func loadSSHKeys() {
        let fileManager = FileManager.default; let sshPath = fileManager.homeDirectoryForCurrentUser.appendingPathComponent(".ssh")
        availableKeys = (try? fileManager.contentsOfDirectory(at: sshPath, includingPropertiesForKeys: nil))?.compactMap { url in
            let name = url.lastPathComponent
            return (name.hasSuffix(".pub") || name == "known_hosts" || name == "config") ? nil : url.path
        } ?? []
    }
    private func save() -> SavedSession {
        let existing = sessionService.sessions.first(where: { $0.id == id })
        let session = SavedSession(id: id, name: name, ipAddress: ipAddress, username: username, keyPath: selectedKeyPath, environment: environment, savedLayouts: existing?.savedLayouts ?? [], customPorts: existing?.customPorts ?? [])
        sessionService.save(session: session); return session
    }
    private func connect() {
        isConnecting = true; let session = save()
        Task {
            let result = await SSHService.testConnection(ip: session.ipAddress, user: session.username, keyPath: session.keyPath)
            await MainActor.run { isConnecting = false; if case .success = result { onConnect(session) } else { SSHService.openTerminalForDebugging(ip: session.ipAddress, user: session.username, keyPath: session.keyPath) } }
        }
    }
}
...
struct PortMappingView: View {
    ...
    var filteredPorts: [RemotePort] {
        let list = searchText.isEmpty ? ports : ports.filter { port in
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack(alignment: .center) { // Alinhamento centralizado rigoroso
                ...
                HStack(spacing: 16) {
                    LayoutMenu(session: session, onApply: applyLayout, onSaveNew: {
                        newLayoutName = ""
            ...
            if let error = tunnelManager.error {
                HStack {
                    ...
                    Text(error).font(.caption); Spacer(); Button("Dismiss") { tunnelManager.error = nil }
                }.padding(10).background(Color.red.opacity(0.1)).foregroundStyle(.red).cornerRadius(8).padding(.horizontal)
            }
            ...
            if isLoading { ProgressView().frame(maxWidth: .infinity, maxHeight: .infinity) }
                ...
                ScrollView {
                    VStack(spacing: 12) {
                        if filteredPorts.isEmpty { Text("No matching services found.").foregroundStyle(.secondary).padding(.top, 20) }
                        else {
                                    ...
                                    TunnelRow(port: $ports[index], onConflictTap: { selectedConflict = ports[index] })
                                        .contextMenu {
                                            if ports[index].isCustom {
                                                Button("Delete Custom Port", role: .destructive) { deleteCustomPort(ports[index]) }
                                            }
                                        }
                                }
                            }
            ...
            VStack(spacing: 0) {
                Divider()
                HStack {
                    Label("\(filteredPorts.count) Services", systemImage: "cube").font(.caption).foregroundStyle(.secondary)
                    Button(action: { showAddCustomPort = true }) { Image(systemName: "plus.circle.fill").foregroundStyle(.secondary) }.buttonStyle(.plain)
                    Spacer()
                    if tunnelManager.isConnected {
                        Button(action: { tunnelManager.stopTunnel() }) {
                            Text("Stop Tunnels").font(.subheadline.bold()).padding(.horizontal, 24).padding(.vertical, 10)
                                .background(Color.red.opacity(0.1)).foregroundStyle(.red).cornerRadius(10)
                                .overlay(RoundedRectangle(cornerRadius: 10).stroke(Color.red.opacity(0.3), lineWidth: 1))
                        }.buttonStyle(.plain)
                    } else {
                        Button(action: { checkAuthAndApply() }) {
                            HStack { if tunnelManager.isConnecting { ProgressView().controlSize(.small) }; Text("Apply Tunnels").font(.subheadline.bold()) }
                        ...
                        .buttonStyle(.plain)
                        .disabled(ports.filter { $0.status == .preActivated }.isEmpty || tunnelManager.isConnecting)
                        .onHover { isHoveringApply = $0 }
                    }
                }.padding().background(.ultraThinMaterial)
            }
        }
        .liquidGlass().onAppear(perform: setupView)
        .alert("Save Layout", isPresented: $showSaveLayoutAlert) {
            TextField("Layout Name", text: $newLayoutName)
            Button("Cancel", role: .cancel) { }; Button("Save") { saveLayout() }
        }
        .sheet(item: $selectedConflict) { port in ConflictSheet(port: String(port.localPort), process: port.status.conflictProcess ?? "Unknown") }
        .sheet(isPresented: $showAddCustomPort) { AddCustomPortSheet { addCustomPort($0) } }
        .sheet(isPresented: $showPasswordPrompt) {
            VStack(spacing: 20) {
                Text("Authentication Required").font(.headline)
                Text("Enter password for \(session.username)@\(session.ipAddress)").font(.caption).foregroundStyle(.secondary)
                SecureField("Password", text: $sshPassword).textFieldStyle(.plain).padding(10).background(Color.primary.opacity(0.05)).cornerRadius(8).frame(width: 250)
                HStack {
                    Button("Cancel") { showPasswordPrompt = false }; Button("Connect") { showPasswordPrompt = false; startTunneling(password: sshPassword) }.buttonStyle(.borderedProminent)
                }
            }.padding(30)
        }
    }
    ...
    private func setupView() {
        loadData()
        tunnelManager.onTermination = { for i in ports.indices { if ports[i].status == .connected { ports[i].status = .preActivated } } }
    }
    ...
    private func loadData() {
        ...
        Task {
            do {
                let containers = try await DockerService.fetchContainers(ip: session.ipAddress, user: session.username, keyPath: session.keyPath)
                var combinedPorts = containers.flatMap { c in c.ports.map { p in RemotePort(containerName: c.name, remotePort: p, localPort: p, status: .disconnected, isCustom: false) }}
                let custom = session.customPorts.map { c in RemotePort(containerName: c.name, remotePort: c.remotePort, localPort: c.localPort, status: .disconnected, isCustom: true, customId: c.id) }
                ...
                await MainActor.run { self.ports = combinedPorts; self.isLoading = false }
            } catch { await MainActor.run { self.isLoading = false } }
        }
    }
    
    private func addCustomPort(_ mapping: CustomPortMapping) { session.customPorts.append(mapping); sessionService.save(session: session); loadData() }
    private func deleteCustomPort(_ port: RemotePort) { guard let id = port.customId else { return }; session.customPorts.removeAll { $0.id == id }; sessionService.save(session: session); loadData() }
    private func saveLayout() {
        ...
        session.savedLayouts.append(PortLayout(name: newLayoutName, mappings: mappings)); sessionService.save(session: session)
    }
    private func applyLayout(_ layout: PortLayout) {
        if tunnelManager.isConnected { return }
        for i in ports.indices { if ports[i].status == .preActivated { ports[i].status = .disconnected } }
        for i in ports.indices {
            if let local = layout.mappings[ports[i].remotePort] {
                ports[i].localPort = local
                let check = PortCheckService.checkPort(local)
                if case .occupied(let proc) = check { ports[i].status = .occupied(process: proc) }
                else { ports[i].status = .preActivated }
            }
        }
    }
    private func checkAuthAndApply() {
        Task {
            let result = await SSHService.testConnection(ip: session.ipAddress, user: session.username, keyPath: session.keyPath)
            await MainActor.run { if case .success = result { startTunneling(password: nil) } else { showPasswordPrompt = true } }
        }
    }
    private func startTunneling(password: String?) {
        ...
        Task {
            ...
            if tunnelManager.isConnected { for i in ports.indices { if ports[i].status == .preActivated { ports[i].status = .connected } } }
        }
    }
}
...
struct StatusIndicator: View {
    ...
    var body: some View {
        ZStack {
            ...
            if active { Circle().stroke(Color.green.opacity(0.5), lineWidth: 4).frame(width: 14, height: 14).blur(radius: 2) }
        }.padding(8).background(.ultraThinMaterial).clipShape(Circle())
    }
}

struct LayoutMenu: View {
    let session: SavedSession; let onApply: (PortLayout) -> Void; let onSaveNew: () -> Void
    var body: some View {
        Menu {
...
struct ExpandableMergingGlassContainer<Content: View, MergedLabel: View>: View {
                ...
                ForEach(subviews: content) { subview in
                    subview
                        ...
                        .visualEffect { content, proxy in
                            content.offset(x: calculateOffset(proxy: proxy, p: progress))
                        }
                        .fixedSize()
                }
            }
    ...
    nonisolated private func calculateOffset(proxy: GeometryProxy, p: CGFloat) -> CGFloat {
...
struct ConflictSheet: View {
    @Environment(\.dismiss) var dismiss; let port: String; let process: String
    var body: some View {
        VStack(spacing: 20) {
            ...
            VStack(spacing: 8) {
                ...
                Text("Local port \(port) is already in use by another application.").font(.subheadline).foregroundColor(.secondary).multilineTextAlignment(.center)
            }
            HStack { Text("Process Name:"); Spacer(); Text(process).bold() }.padding().background(Color.primary.opacity(0.05)).cornerRadius(10)
...
struct TunnelRow: View {
    ...
    private func toggleActivation() {
        withAnimation(.spring()) {
            if port.status == .disconnected { port.status = .preActivated }
            else if port.status == .preActivated { port.status = .disconnected }
        }
    }
}
...
struct AddCustomPortSheet: View {
    @Environment(\.dismiss) var dismiss; @State private var name = ""; @State private var remotePort = ""; @State private var localPort = ""
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            VStack(alignment: .leading) {
                ...
                GlassTextField(icon: "laptopcomputer", title: "Local Port (Default: Same)", text: $localPort)
            }.padding().background(.ultraThinMaterial).cornerRadius(12)
            HStack {
                Button("Cancel") { dismiss() }.keyboardShortcut(.cancelAction)
                Button("Add") {
                    guard let rPort = Int(remotePort), !name.isEmpty else { return }
                    onSave(CustomPortMapping(name: name, remotePort: rPort, localPort: Int(localPort) ?? rPort))
                    dismiss()
...
struct RemotePort: Identifiable {
    let id = UUID(); let containerName: String; let remotePort: Int; var localPort: Int; var status: PortStatus; var isCustom: Bool; var customId: UUID? = nil
}

enum PortStatus: Equatable {
    case connected, disconnected, preActivated, occupied(process: String)
    var themeColor: Color {
        switch self {
        case .connected: return .green; case .disconnected: return .gray
        case .preActivated: return Color.green.opacity(0.6); case .occupied: return .red
        }
    }
    var conflictProcess: String? { if case .occupied(let p) = self { return p }; return nil }
}
