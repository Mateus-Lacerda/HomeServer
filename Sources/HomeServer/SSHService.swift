...
struct SSHService {
  ...
  static func listDirectory(ip: String, user: String, keyPath: String?, path: String) async throws
    -> [String]
  {
    let command = "ls -p1a \"\(path)\""
    let output = try await runCommandDetailed(
      ip: ip, user: user, keyPath: keyPath, command: command)

    if output.exitCode != 0 {
      let errorMsg = output.stderr ?? output.stdout ?? "ls command failed."
      throw NSError(
        domain: "SSH", code: output.exitCode ?? -1, userInfo: [NSLocalizedDescriptionKey: errorMsg])
    }
  ...
  static func readRemoteFile(ip: String, user: String, keyPath: String?, path: String) async throws
    -> String
  {
    ...
    if output.exitCode != 0 {
      let errorMsg = output.stderr ?? output.stdout ?? "cat command failed."
      throw NSError(
        domain: "SSH", code: output.exitCode ?? -1, userInfo: [NSLocalizedDescriptionKey: errorMsg])
    }
    return output.stdout ?? ""
