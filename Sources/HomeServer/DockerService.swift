...
struct DockerService {
    static func fetchContainers(ip: String, user: String, keyPath: String?) async throws -> [DockerContainer] {
        // Format: Name|Ports
        // Example output of docker ps: "web|0.0.0.0:80->80/tcp, :::80->80/tcp"
        let command = "docker ps --format '{{.Names}}|{{.Ports}}'"
        
        let output = try await SSHService.runCommand(ip: ip, user: user, keyPath: keyPath, command: command)
